...
exports.upload = function(local_path, options) {
    ...
    execute(cmd, 'The resource url is: ' + path.join(config.remote_domain, (local_path.slice(-1) === '/' ? options.dest_dir : path.join(options.dest_dir, path.basename(local_path)))).info)
}
