...
var seajs_config = "seajs-config*.js"
...
exports = module.exports = function(src) {
    ...
    child = exec('rm -rf /Users/hwp/Frontend/nodejs/shengyb/static_dist', function (error, stdout, stderr) {
        ...
        seamods.forEach(function(m){
            seamods_hash_map[m] = sha(path.join(static_dist, m))
        })
        // update sea modules
        seamods.forEach(function(m){
            ...
            if(match) {
                ...
                fs.renameSync(file, suffixHash(file, seamods_hash_map[m]));
                console.info('Update and rename: '.info + m)
            }
        })
        //insert sea modules hash map into seajs-config.js
        var seaconf =  glob.sync(seajs_config, {cwd: path.join(project_dir, static_dist)})
        if(seaconf.length){
            seaconf = realpath(path.join(static_dist, seaconf[0])) ;
            var content = fs.readFileSync(seaconf, options.encoding);
            var maps = JSON.stringify(seamods_hash_map, 'null', '\t');
            content = '// sea modules hash map \nseajs.hashmap = ' + maps + '\n\n' + content;
            fs.writeFileSync(seaconf, content);
            console.info('Write hash map of sea modules into: '.info + path.basename(seaconf))
        }
    });
};
